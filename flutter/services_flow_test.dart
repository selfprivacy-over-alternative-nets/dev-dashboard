// Automated L3 flow (dashboard-owned) — SERVICES LIST LOADS.
// Injected into the app's integration_test/ at run time by dash.run_l3_flutter(flow='services'),
// then the Manager repo is restored byte-for-byte.
//
// Flow: launch the real app → wait for it to connect over the transport and reach Providers →
//       tap Services → wait for REAL service tiles (not skeletons) to load.
//
// See providers_flow_test.dart for the hard-won notes (no pumpAndSettle; default frame policy;
// restore ErrorWidget.builder; ffmpeg x11grab screenshots because takeScreenshot is unsupported
// on Linux desktop).
...
void main() {
  ...
  testWidgets('Services list loads real data (automated)', (tester) async {
    try {
      app.main();
      ...
      // REAL data (not the 7 skeleton cards): an actual service NAME must appear.
      ...
      expect(servicesLoaded, isTrue,
          reason: 'Services list never populated with real data within 90s — the backend '
              'AllServices query likely failed.');
    } finally {
